...
class App:
    def __init__(self, root):
        ...
        self.custom_path_checkbox = ctk.CTkCheckBox(root, text="Use custom video path", fg_color="#bf0600", hover_color="#821D1A")
        self.custom_path_checkbox.grid(row=4, column=1, columnspan=2, padx=10, pady=10, sticky="ew")

        self.include_up_next_checkbox = ctk.CTkCheckBox(root, text='Include "UP NEXT"', fg_color="#bf0600", hover_color="#821D1A")
        self.include_up_next_checkbox.grid(row=5, column=1, columnspan=2, padx=10, pady=5, sticky="ew")

        ctk.CTkButton(root, text="Next", command=self.show_events, fg_color="#821D1A", hover_color="#bf0600").grid(row=6, column=1, columnspan=2, pady=10, sticky="ew")
    ...
    def show_events(self):
        ...
        # Only require YouTube link if not using custom path
        if not self.custom_path_checkbox.get():
            if not CHECK.check_youtube(self.youtube_link.get()) or not self.youtube_link.get():
                messagebox.showerror("Error", "Invalid youtube URL")
                return
        
        self.client = Client(self.team_plate.get(), self.youtube_link.get(), self.custom_path_checkbox.get(), self.include_up_next_checkbox.get())
