...
class OCR:
    def __init__(self, client, video_path=None, output_dir=None):
        from src.client import Client
        self.client = client
        # Use provided video_path or construct from client event
        if video_path:
        ...
        # Use provided output directory or fallback to client event
        self.output_dir = output_dir or self.client.event
    ...
    def extract(self, start_time, end_time, output_path):
        ...
        try:
            ...
            compression_thread = threading.Thread(target=self.compress_video, args=(output_path,))
            compression_thread.daemon = True  # Thread will die when main program exits
    ...
    def seek(self, item):
        try:
            ...
            # If include_up_next is disabled, adjust the start time to skip UP NEXT
            if not self.client.include_up_next:
                adjusted_low = self.find_up_next_end_in_clip(low, high)
                if adjusted_low > low:
                    print(f"Adjusted start time from {low}s to {adjusted_low}s to skip UP NEXT")
                    low = adjusted_low - 3 # give us 3 seconds of buffer before countdown
                    
                    # If the adjusted start is too close to or past the end, skip this clip
                    if low >= high - 5:  # Need at least 5 seconds of content
                        print(f"Skipping match {item} - too little content after UP NEXT removal")
                        return None
            ...
            try:
                ...
                pass
            self.extract(low, high, f"{self.output_dir}\\{item}.mp4")

            return low, high
        except KeyError:
            pass

    def find_up_next_end_in_clip(self, start_time, end_time):
        """Find when UP NEXT disappears within a specific clip timeframe"""
        current_time = start_time
        up_next_found = False
        
        while current_time < end_time:
            raw_text = self.get_raw_ocr_text(current_time)
            
            if "UP NEXT" in raw_text.upper():
                up_next_found = True
                print(f"Found \"UP NEXT\" at {current_time}s in clip {start_time}-{end_time}")
            elif up_next_found and "UP NEXT" not in raw_text.upper():
                # UP NEXT was found before but not anymore - it has disappeared
                print(f"\"UP NEXT\" disappeared at {current_time}s in clip {start_time}-{end_time}")
                return current_time

            current_time += 4  # Check every 4 seconds for more precision

        # If UP NEXT was found but never disappeared, return the end time
        if up_next_found:
            print(f"\"UP NEXT\" found but never disappeared in clip {start_time}-{end_time}")
            return end_time
        
        # If UP NEXT was never found, return the original start time
        return start_time

    def get_raw_ocr_text(self, timestamp):
        """Get raw OCR text from a larger area of the video for UP NEXT detection"""
        # Use a larger crop area to catch UP NEXT text
        crop_filter = "crop=iw*0.5:ih*0.3:0:0"  # Larger area than normal OCR
